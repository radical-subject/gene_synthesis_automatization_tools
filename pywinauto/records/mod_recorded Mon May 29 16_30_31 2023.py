...
PWD_LOG_FILES = r"C:\Users\User\Pywinauto recorder"
SLEEP_TIME = 10
CURRENT_PORTION_SCALE_A = "28"
CURRENT_PORTION_SCALE_B = "32"
...
with UIPath(u"Taskbar||Pane"):
    with UIPath(u"Running applications||ToolBar"):
        ...
        send_keys(
            "{VK_TAB}""{VK_TAB}"  # choosing list of files 
            "{VK_DOWN}""{VK_DOWN}"  # moving the selector tp the perticular one 
            "{ENTER}"  # choosing the particular one
            "{VK_TAB}""{VK_TAB}"  # going back to an 'Accept' button
        ...
        send_keys(
            "{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}"  # long way of switches untill the correct panel 'Auto Synthesis Operation'
            "{VK_DOWN}"  # switch the selection frame to the correct panel when the selector is aimed
            "{VK_TAB}""{VK_TAB}"  # switching between buttons
            "{ENTER}"  # chosing the form
        )
        
        lgl.debug("Filling up the form")
        send_keys(
            "{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}"  # chosing the 'Scale A' line
        )
        ...
        send_keys(
            "{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}"  # switching to 'Apply' button
            "{ENTER}"  # press down
        )
        ...
        send_keys(
            "{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}"  # selecting the 'Select protocol' line
            "{ENTER}"  # press down
        )
        ...
        send_keys(
            ...
            "{VK_DOWN}""{VK_DOWN}""{VK_DOWN}"  # chosing the inline-stated protocol file
            "{ENTER}"  # press down 
        )
        lgl.debug("Form 'Select protocol' is done")

        lgl.debug(f"Done with run configuration, waiting {SLEEP_TIME * 2} seconds untill 'Select Oligos appear")
        time.sleep(SLEEP_TIME * 2)
        
        lgl.debug("Selecting 'Select oligos' form")
        send_keys(
            "{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}""{VK_TAB}"  # selecting the button
            "{ENTER}"  # press down 
        )
