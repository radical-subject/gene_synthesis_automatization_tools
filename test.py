...
normalization_coefficient_relative = round(number_of_reactions / normalization_coefficient_min, 5)
for oligo in oligos_table:
 if(oligo[8] <>'Vnorm, ul'):
...
for oligo in oligos_table:
 if(oligo[8] <>'Vnorm, ul'):
    A260_afterSPE_exp = A260_afterSPE_exp +
    float(oligo[5])*float(oligo[3])*float(oligo[8])/elution_volume/1000000
...
with open(result_scheme_file_name,'w') as final_scheme_file:
 final_scheme_file.write('TBIO scheme calculated for ' +
 str(reaction_volume) +
 ' ul reaction volume'+' for ' +
 ...
 for oligo in oligos_table:
    for i in range(0,len(oligo)):
        ...
        final_scheme_file.write('\n')
        final_scheme_file.write('\n')
        final_scheme_file.write('Min Normalization coefficient for volumes =' +
    str(normalization_coefficient_min)+'\n')
    final_scheme_file.write('Relative normalization coefficient for volumes for ' +
    str(int(number_of_reactions))+' reactions = ' +
    str(normalization_coefficient_relative) + '\n')
    final_scheme_file.write('Total volume for reaction = ' +
    str(total_volume_for_reaction) + 'ul' + '\n')
    final_scheme_file.write('Total volume for mixing = ' +
    str (total_volume_for_mixing) + 'ul' + '\n')
    final_scheme_file.write('A260 (1 cm) after SPE with ' +
    str(int(elution_volume)) + ' ul elution = ' +
